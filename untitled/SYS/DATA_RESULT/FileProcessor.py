...
class FileProcessor:
    ...
    @staticmethod
    def in_parquet_uri(in_candidate, is_top, analysis_dir):
        in_candidate = in_candidate.replace(" ", "-")
        parquet_folder = os.path.join(cln_data, "{}/{}-{}.parquet.gzip".format(analysis_dir, is_top, in_candidate))
    ...
    @staticmethod
    def out_parquet_uri(in_candidate="", is_stop=False, out_dir=""):
        in_candidate = in_candidate.replace(" ", "-")
        if in_candidate != "":
            parquet_file = os.path.join(out_dir, "{}-{}.parquet.gzip".format(is_stop, in_candidate))
        else:
            parquet_file = os.path.join(out_dir, "{}.parquet.gzip".format(is_stop))
