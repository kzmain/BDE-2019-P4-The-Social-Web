(function (global, factory) {
	...
	var timelines = function() {
			...
			var hover = function () {},
					...
					tickFormat = {
						format: d3TimeFormat.timeFormat("%m/%d"),
			...
			var appendTimeAxis = function(g, xAxis, yPosition) {
				...
				var axis = g.append("g")
					...
					.attr("transform", "translate(" + 0 + "," + timelines.yMaxValue*y_ratio + ")")
					.attr("dd", "dd")
					.call(xAxis);
  ...
  var clip = gParent.append("defs").append("SVG:clipPath")
      .attr("id", "clip")
      .append("SVG:rect")
      ...
      .attr("y", 0);
				...
				var gX;
				timelines.yMaxValue = d3.max(yData, function(d) { return d.value; });
					...
					d.forEach( function(datum, index){
						...
						view.selectAll("svg")
							...
							.attr("cy", function(d, i) {
								// console.log(getStackPosition(d, i) + itemHeight/2);
						...
						function getStackPosition(d, i) {
							var result;
							if (stacked) {
								result = margin.top + (itemHeight + itemMargin) * yAxisMapping[index];
								// result += (itemHeight + itemMargin) * (chartData.length + 2);
							}else{
								result = margin.top + (itemHeight + itemMargin);
								// result += (itemHeight + itemMargin) * (1 + 2);
							}
							return result + timelines.yMaxValue * y_ratio;
						}
						function getStackTextPosition(d, i) {
							...
							return getStackPosition(d, i) + itemHeight * 0.75;
						}
					});
				...
				g.each(function(d, i){
					...
					view.selectAll("svg")
						...
						.append("rect")
	 					...
	 					.attr("x", getXPos)
	 					.attr("y", function(d) {
							return (timelines.yMaxValue - d.value) * y_ratio;
						})
						...
						.attr("width", (width - 100) / 110)
	 					.attr("height", function(d) {
							return d.value * y_ratio;
						});
				});

				if (width != gParentSize.width) { // only if the scrolling should be allowed
					var move = function() {
						var transform_x = d3Selection.event.transform.x;
						var transform_k = d3Selection.event.transform.k;

						var transform_x_str = "translate(" + transform_x + ",0)";
						var transform_k_str = "scale(" + transform_k+ " 1)"

						g.select(".view")
						.attr("transform", transform_x_str + transform_k_str);
